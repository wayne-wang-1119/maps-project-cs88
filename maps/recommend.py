...
def find_closest(location, centroids):
    ...
    for i in range(0, len(centroids)):
        d.append(distance(centroids[i], location))
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    mx = mean(([restaurant_location(i)[0] for i in cluster]))
    my = mean([restaurant_location(j)[1] for j in cluster])
    return [mx, my]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    sxy =sum([sx[i]*sy[i] for i in range(len(sx))])
    b=sxy/sxx
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    p=[find_predictor(user, reviewed, i) for i in feature_fns]
    keys = {i:j for i,j in p} 
    return max(keys, key = lambda i: keys[i])
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    keys = [restaurant_name(i) for i in restaurants]
    p = {i:j for i,j in zip(keys, [predictor(a) for a in restaurants])}
    nameofreviewed = [restaurant_name(n) for n in reviewed]
    nameofuser = [user_rating(user, m) for m in nameofreviewed]
    p1 = {i:j for i, j in zip(nameofreviewed, nameofuser)}
    p.update(p1)
